...
const NewApplicants = ({appliedCandidates,showApplicants,appliedState}) => {
  ...
  const [showapplied,setShowApplied] = useState(appliedCandidates)
...
useEffect(()=>{
  appliedState(showapplied) 
 
},[showapplied]) 

  useEffect(()=>{
    setShowApplied(appliedCandidates)
},[appliedCandidates])
  ...
  const deleteRejectedApplicant = async()=>{
      ...
      const {data} = await axios.put(process.env.REACT_APP_SERVER_URL + `company/rejectedapplicants/${iid}?type=Shortlisted`,[
        ...showapplied
    ])
    if(data.status){
      showApplicants()
  ...
  return (
    ...
    (application.internshipstatus === "Applied"?(
